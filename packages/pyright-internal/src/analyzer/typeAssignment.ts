...
export function narrowTypeBasedOnAssignment(
    ...
    const narrowedType = mapSubtypes(assignedTypeResult.type, (assignedSubtype) => {
        ...
        const narrowedSubtype = mapSubtypes(declaredType, (declaredSubtype) => {
            if (!assignType(evaluator, registry, state, declaredSubtype, assignedSubtype)) {
                return undefined;
            }
            ...
            if (assignType(evaluator, registry, state, assignedSubtype, declaredSubtype)) {
...
export function validateOverrideMethod(
    evaluator: TypeEvaluator,
    registry: TypeRegistry,
    state: TypeEvaluatorState,
    baseMethod: Type,
    ...
    if (isFunction(overrideMethod)) {
        return OverloadedType.getOverloads(baseMethod).every((overload) => {
            ...
            if (baseClass && !isOverrideMethodApplicable(evaluator, registry, state, overload, baseClass)) {
                return true;
            }
    ...
    for (const overrideOverload of OverloadedType.getOverloads(overrideMethod)) {
        ...
        let matchIndex = baseOverloads.findIndex((baseOverload, index) => {
            // If the override isn't applicable for this base class, skip the check.
            if (baseClass && !isOverrideMethodApplicable(evaluator, registry, state, baseOverload, baseClass)) {
                return false;
            }
    ...
    if (previousMatchIndex < baseOverloads.length - 1) {
        ...
        if (
            ...
            unmatchedOverloads.some((overload) => {
                return isOverrideMethodApplicable(evaluator, registry, state, overload, baseClass);
            })
        ) {
...
function isOverrideMethodApplicable(
    evaluator: TypeEvaluator,
    registry: TypeRegistry,
    state: TypeEvaluatorState,
